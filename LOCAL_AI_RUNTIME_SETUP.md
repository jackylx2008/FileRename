...
LLAMACPP_AUTOSTART=true
LLAMACPP_SERVER_PATH=D:\path\to\llama-cpp-cu12\llama-server.exe
LLAMACPP_MODEL_PATH=C:\Users\your_name\.lmstudio\models\lmstudio-community\Qwen3.6-27B-GGUF\Qwen3.6-27B-Q4_K_M.gguf
LLAMACPP_MMPROJ_PATH=C:\Users\your_name\.lmstudio\models\lmstudio-community\Qwen3.6-27B-GGUF\mmproj-Qwen3.6-27B-BF16.gguf
LLAMACPP_EXTRA_DLL_DIRS=./vendor/cuda12
LLAMACPP_N_GPU_LAYERS=999
LLAMACPP_CTX_SIZE=8192
LLAMACPP_REASONING=off
...
3. 请求 `GET /health`
4. 请求 `GET /v1/models`
5. 如果服务不可用且 `LLAMACPP_AUTOSTART=true`，自动启动 `llama-server`
6. 轮询等待服务可用
7. 校验配置模型名是否存在于模型列表
8. 对图片逐一调用 `POST /v1/chat/completions`
...
```powershell
$env:PATH=(Resolve-Path ".\vendor\cuda12").Path + ";D:\path\to\llama-cpp-cu12;" + $env:PATH
& "D:\path\to\llama-cpp-cu12\llama-server.exe" --list-devices
```
...
load_backend: loaded CUDA backend from D:\path\to\llama-cpp-cu12\ggml-cuda.dll
Available devices:
...
```powershell
.\.venv\Scripts\python.exe image_index_extract.py --help
```

准备好 `common.env`、图片目录和本地模型服务后，可只处理一张图片做端到端验证：

```powershell
.\.venv\Scripts\python.exe image_index_extract.py --limit 1 --max-tokens 4096
```

成功时会输出包含以下字段的汇总 JSON：

```json
{
  "input_dir": "C:\\path\\to\\pngs",
  "image_count": 1,
  "item_count": 1,
  "error_count": 0,
  "json_path": "output\\image_index_extract\\image_index_results.json"
}
```
...
`src/localai/flows/image_index_extract.py` 已按此方式处理。后续需要调用本地模型的 flow 也应沿用同样的生命周期管理方式。

## 10. 新入口脚本接入方式

新增一个根目录入口脚本时，应保持入口层很薄：

1. 将项目根目录下的 `src/` 加入 `sys.path`
2. 通过 `load_runtime_config(...)` 读取 `common.env` 和 `config.yaml`
3. 初始化统一日志
4. 调用 `src/localai/flows/` 中对应的 `run(...)`
5. 用 `print_json(...)` 输出结构化结果
